export class Base {
     ...
     * } 
        ...
        mode: "hash"
        fallback: false
        history: HashHistory {router: Router, current: {…}}
     */

    constructor (router) {
      this.router = router
      console.log('base.js thisFinite.router',this.router);
      this.current = {
        path: '/',
        query: {},
        params: {},
    ...
    transitionTo(target, cb) {
      //targte为当前锚点，例如 bar, cb为成功回调
      // 通过对比传入的 routes 获取匹配到的 targetRoute 对象
      const targetRoute = match(target, this.router.routes)
      //获取路由匹配 例 targetRoute  {path: "/foo", name: "foo", component: "<div>Foo</div>"}
    ...
    confirmTransition (route, cb) {
      ...
      console.log('confirmTransition',this.router.beforeEach,this.current.route.beforeLeave,route.beforeEnter,route.afterEnter)
